...
if not fireproximityprompt then
    ...
    msg.Text = "you have fireproximityprompt function bro get better executor. (Like Comet!!!)"
    task.wait(6)
    msg:Destroy()
    error("no prox") 
end
...
local window_quit = library.window("quit")
    ...
    if val then
        ...
        addconnect = workspace.ChildAdded:Connect(function(v)
            if table.find(entitynames,v.Name) then
                ...
                game:GetService("TextChatService"):WaitForChild("TextChannels"):WaitForChild("RBXGeneral"):SendAsync(v.Name:gsub("Moving",""):lower().." is coming go hide")
...
window_quit.button("close gui",function()
    flags = DELFLAGS
...
window_credits.label("Credits: Catteleya Twiter:\n@cattsdevteam",20)
